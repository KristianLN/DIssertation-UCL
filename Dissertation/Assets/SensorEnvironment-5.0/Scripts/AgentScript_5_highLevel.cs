...
public class AgentScript_5_highLevel : Agent
{
    ...
    public override void AgentAction(float[] vectorAction,string textAction)
    {
      ...
      // All movement happens to the invisible agent.

      // Rotate the invisible agent
      invisibleAgent.transform.Rotate(rotateDir,Time.deltaTime * 150f);
      //
      // // Move
      invisibleAgent.GetComponent<Rigidbody>().velocity = transform.forward * speed;
    ...
    public void OnCollisionEnter(Collision collision)
    {
      if (collision.collider.CompareTag("Wall"))
      {
        ...
        Done();
        // Resetting the invisible agent as well.
        GameObject.FindWithTag("InvisibleAgent").GetComponent<AgentScript_5_ArtificialAgent>().AgentReset();
      }
      ...
      if ( collision.collider.CompareTag("Obstacle"))
      {
        ...
        Done();
        // Resetting the invisible agent as well.
        GameObject.FindWithTag("InvisibleAgent").transform.position = startPos;
        GameObject.FindWithTag("InvisibleAgent").transform.eulerAngles = startRot;
        GameObject.FindWithTag("InvisibleAgent").GetComponent<Rigidbody>().velocity = Vector3.zero;
        GameObject.FindWithTag("InvisibleAgent").GetComponent<Rigidbody>().angularVelocity = Vector3.zero;
      }
    ...
    public void Update()
    {
      // Not needed unless the reward signal should be less sparse than normal.
      // if (RewardFrequency != -1)
      // {
      //   // If the reward signal is chosen to be less sparse than the traditional
      //   // Not done!!!
      //   if (intendedPath.Count < RewardFrequency && realisedPath.Count < RewardFrequency)
      //   {
      //     // Adding the position suggested by the high-level agent
      //     intendedPath.Add(invisibleAgent.transform.position);
      //     realisedPath.Add(transform.position);
      //   } else
      //   {
      //     Debug.Log("Hej");
      //   }
      // } else
      // {
      //   // Adding evey position realised and intended.
      //   intendedPath.Add(invisibleAgent.transform.position);
      //   realisedPath.Add(transform.position);
      // }
      // if (secondBrain != null)
