...
public class AgentScript_4_3 : Agent
{
    ////////////////////////////////// Notes //////////////////////////////////
    ...
    AcademyScript_4_3 academyScript;
    targetPlacing_4_3 targetScript;
    // Multiple brains
    ...
    public override void InitializeAgent()
    {
      ...
      firstBrain = brain;
      // Debug.Log(brain.brainParameters.vectorObservationSpaceSize);

      // Storing our agent for later use
      agent = this.GetComponent<Agent>();
      // Get scripts from other objects.
      academyScript = GameObject.FindWithTag("Academy").GetComponent<AcademyScript_4_3>();
      parentObject = this.transform.parent.gameObject;
      firstChild = parentObject.transform.GetChild(0).gameObject;
      targetScript = firstChild.transform.GetChild(0).GetComponent<targetPlacing_4_3>();
      // Getting the number of areas
      numberOfAreas = GameObject.FindGameObjectsWithTag("Area").Length;

      if (drawTrails)
      {
        bool drawingScript = GetComponent<drawDynamicTrail_4_3>().enabled = true;
      } else
      {
        bool drawingScript = GetComponent<drawDynamicTrail_4_3>().enabled = false;
      }
      // Initialising variables
      ...
      if (resetLocations)
      {
        ...
        if ((int)academyScript.resetParameters["NumberOfSensorClouds"] != 0)
        {
          ...
          for (int j = 0; j < academyScript.resetParameters["NumberOfSensorClouds"];j++)//NumberofSensorClouds
          {
            ...
            randomPosition.y = movementHeight;
    ...
    void OnTriggerEnter(Collider collider)//private
    {
      ...
      if (collider.gameObject.CompareTag("CrowdedArea"))//without gameObject
      {
        ...
        agent.GetComponent<AgentScript_4_3>().speed = 2.5f;
      }
    }
    void OnTriggerExit(Collider collider)//private
    {
      if (collider.gameObject.CompareTag("CrowdedArea"))//without gameObject
      {
        ...
        slowSteps_obs = 0;
        stepPenalty = -0.0005f;
        agent.GetComponent<AgentScript_4_3>().speed = 10f;
      }
